...
VehicleGazeboInfoPublisher::VehicleGazeboInfoPublisher() : nh_(""), pnh_("~"), tf_listener_(tf_buffer_)
{
    ...
    odom_sub_ = nh_.subscribe("/sd_twizy/joint_states", 1, &VehicleGazeboInfoPublisher::jointStateCallback, this);
    // Define the transformation between the "world" and "map" frames
    geometry_msgs::TransformStamped world_to_map_transform;
    world_to_map_transform.header.stamp = ros::Time::now();
    world_to_map_transform.header.frame_id = "world";
    world_to_map_transform.child_frame_id = "map";
    world_to_map_transform.transform.translation.x = 0.0;
    world_to_map_transform.transform.translation.y = 0.0;
...
void VehicleGazeboInfoPublisher::publishTimerCallback(const ros::TimerEvent &e)
{
    ...
    base_link_srv.request.reference_frame = "world";
    ros::Time current_time = ros::Time::now();

    // Check the result of the service call
    if (client_.call(base_link_srv))
    {
    ...
    {
        ROS_WARN("Failed to call service /gazebo/get_link_state");
    }
}
