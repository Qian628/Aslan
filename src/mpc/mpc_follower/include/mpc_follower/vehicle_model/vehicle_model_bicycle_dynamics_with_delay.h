...
/**
 * @file vehicle_model_bicycle_dynamics_with_delay.h
 * @brief vehicle model class of bicycle dynamics including 1st order delay in the steering input
 * @author Qian Lu
 * @date 2023/05/23
 */
 ...
 * v           : velocity 
 * steer_tau   : time constant of the 1st order delay in the steering input
 * m           : mass
 * Iz          : inertia
 ...
 * 
 *    State & Input
 * x = [e, de, th, dth, steer]^T
 * u = steer
 * 
 *    Linearized model around reference point (v=vr)
 *          [0,                   1,                0,                        0,             0]       [          0]       [                          0]
 *  dx/dt = [0,       -(cf+cr)/m/vr,        (cf+cr)/m,       (lr*cr-lf*cf)/m/vr,          cf/m] * x + [          0] * u + [(lr*cr-lf*cf)/m/vr*k - vr*k]
 *          [0,                   0,                0,                        1,             0]       [          0]       [                          0]
 *          [0, (lr*cr-lf*cf)/Iz/vr, (lf*cf-lr*cr)/Iz, -(lf^2*cf+lr^2*cr)/Iz/vr,      lf*cf/Iz]       [          0]       [   -(lf^2*cf+lr^2*cr)/Iz/vr]
 *          [0,                   0,                0,                        0,  -1/steer_tau]       [1/steer_tau]       [                          0]
 ...
 */
class DynamicsBicycleModelWithDelay : public VehicleModelInterface
{
public:
  /**
   * @brief constructor with parameter initialization
   * @param [in] wheelbase wheelbase length [m]
   * @param [in] steer_tau time constant of steering input delay [s]
   ...
   */
  DynamicsBicycleModelWithDelay(double &wheelbase, double &steer_tau, double &mass_fl, double &mass_fr,
                       double &mass_rl, double &mass_rr, double &cf, double &cr);

  /**
   * @brief destructor
   */
  ~DynamicsBicycleModelWithDelay() = default;
...
private:
  ...
  double steer_tau_; //!< @brief time constant of steering input delay [s]
